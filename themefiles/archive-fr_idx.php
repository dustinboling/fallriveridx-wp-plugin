...
/* Get params */
if ( isset ( $wp_query->query_vars[ 'fridx_listing' ] ) ) {
    // enqueue scripts
    $nivo_css = plugins_url('fallriveridx/js/nivo-slider/nivo-slider.css');
    $nivo_theme = plugins_url('fallriveridx/js/nivo-slider/themes/default/default.css');
    $nivo_jq_js = plugins_url('fallriveridx/js/nivo-slider/jquery.nivo.slider.pack.js');
    $nivo_js = plugins_url('fallriveridx/js/fridx-listing.js');
?>
    <link href="<?php echo $nivo_css ?>" rel="stylesheet" type="text/css" media="all">  
    <link href="<?php echo $nivo_theme ?>" rel="stylesheet" type="text/css" media="all">
    <script type="text/javascript" src="<?php echo $nivo_jq_js ?>"></script>
    <script type="text/javascript" src="<?php echo $nivo_js ?>"></script>
<?php
    // get listings
    $address = fridx_capitalize_query( $wp_query->query_vars[ 'fridx_listing' ] );
    $jsonurl = "http://fallriveridx.heroku.com/api/properties/show.json?Token=";
    $jsonurl .= FALL_RIVER_TOKEN;
    $jsonurl .= "&ListingID=" . fridx_parse_listing_id( $address );
    ...
    if ( isset ( $json_output[ 'response' ][ 'success' ] ) && $json_output[ 'response' ][ 'success' ] == false) {
        ...
        $fr_error_msg = "Listing not found.";
    } else {
        $listing = $json_output[0][ 'listing' ];
    } 
} else {
    return "ListingID has not been supplied";
}
...
<div id="show-listing-container" class="fridx-container">
    <h1 id=""><?php echo fridx_titleize( $listing[ 'FullStreetAddress' ] ); ?></h1>
    ...
    <div id="fridx-disclaimer" class="fridx-disclaimer">
        <?php echo fridx_disclaimer() ?>
    </div>
</div>
